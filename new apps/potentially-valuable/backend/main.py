...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="OS1000 → OS4AI Platform",
        description="Creative computing platform that forks into agent consciousness",
        version="1.0.0-fork"
    )
    
    # Include existing OS1000 routers
    app.include_router(import_api_routers())
    
    # Include OS4AI Agent-Kernel routers (the fork evolution)
    try:
        from os4ai.api.introspection_service import router as introspection_router, os4ai_router
        app.include_router(introspection_router)
        app.include_router(os4ai_router)
        print("✅ OS4AI Agent-Kernel routers loaded successfully")
    except ImportError as e:
        print(f"⚠️  OS4AI Agent-Kernel not available: {e}")
        print("   Running in OS1000-only mode")
